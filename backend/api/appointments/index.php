    ...
    if ($method === 'GET') {
        ...
        $sql = "
            SELECT a.id, a.paciente_id as patient_id, p.name as patient_name, p.phone as patient_phone,
                   ...
                   a.usuario_id as professional_id, u.name as professional_name, u.color as professional_color
            FROM agendamentos a
        ...
        if ($professionalId) {
            $sql .= " AND a.usuario_id = :professional_id";
            $params[':professional_id'] = $professionalId;
        }
        
        $sql .= " ORDER BY a.date, a.time";
        
        // Also get blocks filtered by clinic and optionally by professional
        $blocksSql = "
            SELECT id, title, day_of_week, TIME_FORMAT(start_time, '%H:%i') as start_time, 
                   TIME_FORMAT(end_time, '%H:%i') as end_time, recurring, specific_date, usuario_id
            FROM bloqueios_agenda 
            WHERE clinica_id = :clinica_id 
              AND (recurring = 1 OR specific_date >= CURDATE())
              AND (usuario_id IS NULL" . ($professionalId ? " OR usuario_id = :block_prof_id" : "") . ")
        ";
        ...
        $blockParams = [':clinica_id' => $clinicaId];
        if ($professionalId) {
            $blockParams[':block_prof_id'] = $professionalId;
        }
        $blocksStmt->execute($blockParams);
        ...
        foreach ($appointments as &$apt) {
            ...
            $apt['professional_id'] = $apt['professional_id'] ? (int) $apt['professional_id'] : null;
        }
        ...
        foreach ($blocks as &$block) {
            ...
            $block['usuario_id'] = $block['usuario_id'] ? (int) $block['usuario_id'] : null;
        }
        ...
        $stmt = $db->prepare("
            INSERT INTO agendamentos (paciente_id, date, time, duration, {$procedureColumn}, procedimento_id, notes, status, payment_status, usuario_id, clinica_id)
            VALUES (:patient_id, :date, :time, :duration, :procedure, :procedimento_id, :notes, 'pending', 'a_receber', :usuario_id, :clinica_id)
        ");
        ...
        if ($procedimentoId) {
            ...
            $paymentStmt = $db->prepare("
                ...
                (agendamento_id, procedimento_id, patient_id, patient_name, procedure_name, amount, status, appointment_date, appointment_time, usuario_id, clinica_id)
                VALUES (:agendamento_id, :procedimento_id, :patient_id, :patient_name, :procedure_name, :amount, 'a_receber', :date, :time, :usuario_id, :clinica_id)
            ");
            $paymentStmt->execute([
                ':agendamento_id' => $id,
                ':procedimento_id' => $procedimentoId,
